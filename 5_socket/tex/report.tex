...
これは、えーどうしようか　synchronizedを使う必要、ある？　調べようね　スレッドセーフについて

\begin{lstlisting}[caption=マルチスレッドの例3,label=mt3]
  import java.io.*;
  import java.util.ArrayList;
  public class Main {
    public static void main(String[] args) {
      ArrayList<Integer> ary = new ArrayList<>();
      hoge fuga = new hoge(ary);
      Thread t[] = new Thread[10];
      for (int i = 0; i < 10; ++i) t[i] = new Thread(fuga);
      for (int i = 0; i < 10; ++i) t[i].start();
      for (int i = 0; i < 10; ++i)
      ...
      for (Integer i : fuga.getAry()) 
        System.out.println("main : " + i);
    }
  }
  class hoge implements Runnable {
    private ArrayList<Integer> ary;
    public hoge(ArrayList<Integer> a) {this.ary = a;}
    public void run() {
      BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
      try {
        this.ary.add(Integer.parseInt(br.readLine()));
      } catch (IOException e) {
        e.printStackTrace();
      }
    }
    public ArrayList<Integer> getAry() {return this.ary;}
  }
\end{lstlisting}
